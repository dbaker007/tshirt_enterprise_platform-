...
# Import your private shipping data access tracking assets
from db import SessionLocal, ShippingOutbox, init_shipping_db
...
logger = logging.getLogger("SHIPPING_DAEMON")
...
KAFKA_CONFIG = {
    ...
    "client.id": "shipping_avro_outbox_daemon",
    "acks": "all",
}
...
def process_shipping_outbox_ledger() -> int:
    """Polls private shipping outbox table, serializes via Avro, and streams out-of-band."""
    db = SessionLocal()
    try:
        ...
        pending_events = (
            db.query(ShippingOutbox).order_by(ShippingOutbox.id.asc()).limit(20).all()
        )
...
if __name__ == "__main__":
    logger.info(
        "Shipping Background Outbox Streaming Worker Daemon Initialized Symmetrically."
    )
    try:
        while True:
            sleep_duration = process_shipping_outbox_ledger()
            if sleep_duration == 0:
                time.sleep(2)
            else:
