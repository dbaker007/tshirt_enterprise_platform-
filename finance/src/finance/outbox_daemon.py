...
# Import your private finance data access tracking assets
from db import FinanceOutbox, SessionLocal, init_finance_db
...
# DRY COMPLIANCE: Two-hop path resolution to load the shared SagaReply contract from root schemas/ hub
...
KAFKA_CONFIG = {
    ...
    "client.id": "finance_avro_outbox_daemon",
    "acks": "all",
}
...
def process_finance_outbox_ledger() -> int:
    """Polls private finance outbox table, serializes via Avro, and streams out-of-band."""
    db = SessionLocal()
    try:
        # Enforce structural integrity using our data layer initializer
        init_finance_db()

        pending_events = (
            db.query(FinanceOutbox).order_by(FinanceOutbox.id.asc()).limit(20).all()
        )
...
if __name__ == "__main__":
    logger.info(
        "Finance Background Outbox Streaming Worker Daemon Initialized Symmetrically."
    )
    try:
        while True:
            sleep_duration = process_finance_outbox_ledger()
            if sleep_duration == 0:
                time.sleep(2)
            else:
